####################################################################################################
# POST-PROCESSING FOR SS1 SCENARIO D
####################################################################################################
...
for (sim in 1:10) {
  # load(paste0("Simulation_Study_1/Scripts_Results/SS1D_Output/Output_SS1D_Simulation_", sim, ".Rdata"))
  load(paste0("/Volumes/My Passport/PhD/PhD Project 1/LaPCoM_Code/Simulation_Study_1/Scripts_Results/SS1D_Output/Output_SS1D_Simulation_", sim, ".Rdata"))
...
# SET THE SEED 
# ------------------------------------------------------------------------------------------------
seed = seeds_study_1[34]
set.seed(seed)

# ------------------------------------------------------------------------------------------------
# POST-PROCESSING
# ------------------------------------------------------------------------------------------------
pp_res_D = vector("list", length = 2)
pp_res_D$without_node_clustering = post_process_monoLaPCM(output = output_combined_monoLaPCM, 
                                                       multi = output_combined[[1]]$sim_multi$multi,
                                                       cols = khroma::color("light")(9),
                                                       plots_path = "Simulation_Study_1/Post_Processing/SS1D_PP/WITHOUT_NC")
cols_index = c(1:DescTools::Mode(pp_res_D$without_node_clustering$posterior_means_modes$G_plus_hat)[1])
pp_res_D$with_node_clustering = post_process_LaPCoM(output = output_combined_LaPCoM, 
                                                multi = output_combined[[1]]$sim_multi$multi,
                                                cols = khroma::color("light")(9)[-cols_index],
                                                plots_path = "Simulation_Study_1/Post_Processing/SS1D_PP/WITH_NC")
save(pp_res_D, file = "Simulation_Study_1/Post_Processing/SS1D_PP/pp_res_D.Rdata")  
