...
# ==================================================================================================
# LOAD IN ALL NECESSARY FILES AND FUNCTIONS FOR SIMULATION STUDY 1 SCENARIO A
# ==================================================================================================
source("Seeds_Study_1.R")
source("LaPCoM_Generate_Data.R")
source("LaPCoM_Initialisation_Functions.R")
source("LaPCoM_FCs.R")
source("mono_LaPCM_FCs.R")
source("LaPCoM.R")
source("mono_LaPCM.R")
...
output_SS1A = foreach::foreach(sim = 1:10) %dopar% {
  
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  # SET THE SEED
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  seed = seeds_study_1[1:10][sim]
  set.seed(seed)
  
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  # CREATE A LOG FILE
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  log_file = paste0("SS1A_Logs/Output_SS1A_Simulation", sim, ".txt")
  ...
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  res_without_node_clusters = mono_LaPCM(multi = sim_multi$multi,
                                     net_type = net_type,
                                     net_mode = net_mode,
                                     G_max = G_max,
                                     s_e = s_e,
  ...
  save(output, file = paste0("SS1A_Output/Output_SS1A_Simulation_", sim, ".Rdata"))
} # end foreach loop
