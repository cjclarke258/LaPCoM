####################################################################################################
# SIMULATION STUDY 2 - SCENARIO V - VIA REBAFKA
####################################################################################################
rm(list = ls())
...
output_SS2_SV_Rebafka = foreach::foreach(sim = 1:10) %dopar% {
  ...
  # CREATE A LOG FILE
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  log_file = paste0("Simulation_Study_2/Results/SS2_SV_Rebafka/SS2_SV_Rebafka_Sim", sim, "_log.txt")
  ...
  save(output, file = paste0("Simulation_Study_2/Results/SS2_SV_Rebafka/Output_SS2_SV_Rebafka_Simulation_", 
                             sim, ".Rdata"))
