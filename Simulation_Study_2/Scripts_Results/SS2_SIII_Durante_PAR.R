####################################################################################################
# SIMULATION STUDY 2 - SCENARIO III - VIA DURANTE
####################################################################################################
rm(list = ls())
...
output_SS2_SIII_Durante = foreach::foreach(sim = 1:10) %dopar% {
  ...
  # CREATE A LOG FILE
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  log_file = paste0("Simulation_Study_2/Results/SS2_SIII_Durante/SS2_SIII_Durante_Sim", sim, "_log.txt")
  ...
  save(output, file = paste0("Simulation_Study_2/Results/SS2_SIII_Durante/Output_SS2_SIII_Durante_Simulation_", 
                             sim, ".Rdata"))
