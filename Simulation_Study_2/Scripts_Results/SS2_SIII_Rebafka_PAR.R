####################################################################################################
# SIMULATION STUDY 2 - SCENARIO III - VIA REBAFKA
####################################################################################################
rm(list = ls())
...
output_SS2_SIII_Rebafka = foreach::foreach(sim = 1:10) %dopar% {
  ...
  # CREATE A LOG FILE
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  log_file = paste0("Simulation_Study_2/Results/SS2_SIII_Rebafka/SS2_SIII_Rebafka_Sim", sim, "_log.txt")
  ...
  save(output, file = paste0("Simulation_Study_2/Results/SS2_SIII_Rebafka/Output_SS2_SIII_Rebafka_Simulation_", 
                             sim, ".Rdata"))
