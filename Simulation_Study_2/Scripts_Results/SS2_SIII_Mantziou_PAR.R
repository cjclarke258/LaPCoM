####################################################################################################
# SIMULATION STUDY 2 - SCENARIO III - VIA MANTZIOU
####################################################################################################
rm(list = ls())
...
output_SS2_SIII_Mantziou = foreach::foreach(sim = 1:10) %dopar% {
  ...
  # CREATE A LOG FILE
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  log_file = paste0("Simulation_Study_2/Results/SS2_SIII_Mantziou/SS2_SIII_Mantziou_Sim", sim, "_log.txt")
  ...
  save(output, file = paste0("Simulation_Study_2/Results/SS2_SIII_Mantziou/Output_SS2_SIII_Mantziou_Simulation_", sim, ".Rdata"))
