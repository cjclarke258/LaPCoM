####################################################################################################
# SIMULATION STUDY 2 - SCENARIO II - VIA MANTZIOU
####################################################################################################
rm(list = ls())
...
output_SS2_SII_Mantziou = foreach::foreach(sim = 1:10) %dopar% {
  ...
  # CREATE A LOG FILE
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  log_file = paste0("Simulation_Study_2/Results/SS2_SII_Mantziou/SS2_SII_Mantziou_Sim", sim, "_log.txt")
  ...
  save(output, file = paste0("Simulation_Study_2/Results/SS2_SII_Mantziou/Output_SS2_SII_Mantziou_Simulation_", sim, ".Rdata"))
