####################################################################################################
# SIMULATION STUDY 2 - SCENARIO II - VIA SIGNORELLI
####################################################################################################
rm(list = ls())
...
# STORAGE SET UP
# ------------------------------------------------------------------------------------------------
output_SS2_SII_Signorelli = list()
...
output_SS2_SII_Signorelli = foreach::foreach(sim = 1:10) %dopar% {
  ...
  # CREATE A LOG FILE
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  log_file = paste0("Simulation_Study_2/Results/SS2_SII_Signorelli/SS2_SII_Signorelli_Sim", sim, "_log.txt")
  ...
  save(output, file = paste0("Simulation_Study_2/Results/SS2_SII_Signorelli/Output_SS2_SII_Signorelli_Simulation_", 
                             sim, ".Rdata"))
