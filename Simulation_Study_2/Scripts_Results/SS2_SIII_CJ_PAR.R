####################################################################################################
# SIMULATION STUDY 2 - SCENARIO III - VIA OUR MCMLPM
####################################################################################################
rm(list = ls())
...
output_SS2_SIII_CJ = foreach::foreach(sim = 1:10) %dopar% {
  ...
  # CREATE A LOG FILE
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  log_file = paste0("SS2_SIII_CJ/SS2_SIII_CJ_Sim", sim, "_log.txt")
  ...
  save(output, file = paste0("SS2_SIII_CJ/Output_SS2_SIII_CJ_Simulation_", sim, ".Rdata"))
