####################################################################################################
# SIMULATION STUDY 2 - SCENARIO V - VIA SIGNORELLI
####################################################################################################
rm(list = ls())
...
output_SS2_SV_Signorelli = foreach::foreach(sim = 1:10) %dopar% {
  ...
  # CREATE A LOG FILE
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  log_file = paste0("Simulation_Study_2/Results/SS2_SV_Signorelli/SS2_SV_Signorelli_Sim", sim, "_log.txt")
  ...
  save(output, file = paste0("Simulation_Study_2/Results/SS2_SV_Signorelli/Output_SS2_SV_Signorelli_Simulation_", 
                             sim, ".Rdata"))
