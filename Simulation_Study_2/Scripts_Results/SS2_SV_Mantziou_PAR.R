####################################################################################################
# SIMULATION STUDY 2 - SCENARIO V - VIA MANTZIOU
####################################################################################################
rm(list = ls())
...
output_SS2_SV_Mantziou = foreach::foreach(sim = 1:10) %dopar% {
  ...
  # CREATE A LOG FILE
  # ``````````````````````````````````````````````````````````````````````````````````````````````
  log_file = paste0("Simulation_Study_2/Results/SS2_SV_Mantziou/SS2_SV_Mantziou_Sim", sim, "_log.txt")
  ...
  save(output, file = paste0("Simulation_Study_2/Results/SS2_SV_Mantziou/Output_SS2_SV_Mantziou_Simulation_", sim, ".Rdata"))
