########################################################################################################################
# KRACKHARDT DATA APPLICATION (POSTERIOR PREDICTIVE CHECKS)
########################################################################################################################
...
# LaPCoM RESULTS
load("pp_res_krack_PPMethodC.Rdata")
...
best_chain_num = 25
num_PPCs = 500
num_samps = dim(krack_pp$pp_res_krack$chains$Z[[best_chain_num]])[4]

LaPCoM_PPC_alphas = tail(krack_pp$pp_res_krack$chains$alpha[[best_chain_num]], num_PPCs)
LaPCoM_PPC_Z = krack_pp$pp_res_krack$chains$Z[[best_chain_num]][, , , tail(1:num_samps, num_PPCs)]
LaPCoM_PPC_C = krack_pp$pp_res_krack$chains$C[[best_chain_num]][, tail(1:num_samps, num_PPCs)]
...
multis_PPC = probs = array(NA, dim = c(N, N, M, num_PPCs))
...
for (ppc_i in 1:num_PPCs) {
  ...
  for (m in 1:M) {
    ...
    # ----------------------------------------------------------------------------------------------------------------
    pred_obj_nets = ROCR::prediction(as.vector(multis_PPC[, , m, ppc_i]), as.vector(krack_multi[, , m]))
    ...
    gra2 = igraph::graph_from_adjacency_matrix(multis_PPC[, , m, ppc_i], "directed")
    ...
    # F1-SCORE
    # ----------------------------------------------------------------------------------------------------------------
    F1s[m, ppc_i] = MLmetrics::F1_Score(krack_multi[, , m], multis_PPC[, , m, ppc_i])
...
save.image(file = "LaPCoM_Krack_PPCs.Rdata")
beepr::beep(sound = 10)
...
png("Krack_PPC_Plots/Updated_Krack_PPC_Density.png", height = 420 * 2, width = 480 * 3)
par(mfrow = c(3, 7))
par(mar = c(5, 5, 4, 1) + 0.1) # bltr
for (m in 1:M) { 
  boxplot(as.vector(square_diff_densities[m, ]), xlab = "", ylab = "", ylim = c(0, 0.3),
...
png("Krack_PPC_Plots/Updated_Krack_PPC_AUC_probs.png", height = 420 * 2, width = 480 * 3)
par(mfrow = c(3, 7))
par(mar = c(5, 5, 4, 1) + 0.1) # bltr
for (m in 1:M) {
...
png("Krack_PPC_Plots/Updated_Krack_PPC_AUC_nets.png", height = 420 * 2, width = 480 * 3)
par(mfrow = c(3, 7))
par(mar = c(5, 5, 4, 1) + 0.1) # bltr
for (m in 1:M) {
...
png("Krack_PPC_Plots/Updated_Krack_PPC_Net_Distances.png", height = 420 * 2, width = 480 * 3)
par(mfrow = c(3, 7))
par(mar = c(5, 5, 4, 1) + 0.1) # bltr
for (m in 1:M) {
  boxplot(as.vector(net_dists[m, ]), xlab = "", ylab = "", ylim = c(0, 0.5),
          main = paste("Network", LETTERS[m]), las = 1, cex.axis = 2, cex.lab = 1.5, cex.main = 2) 
  legend("top", legend = paste("Median =", round(median(net_dists[m, ]), 2)), bty = "n", cex = 1.5)
} # end m for loop
dev.off()

png("Krack_PPC_Plots/Updated_Krack_PPC_Ham_Distances.png", height = 420 * 2, width = 480 * 3)
par(mfrow = c(3, 7))
par(mar = c(5, 5, 4, 1) + 0.1) # bltr
for (m in 1:M) {
...
png("Krack_PPC_Plots/Updated_Krack_PPC_F1s.png", height = 420 * 2, width = 480 * 3)
par(mfrow = c(3, 7))
par(mar = c(5, 5, 4, 1) + 0.1) # bltr
for (m in 1:M) {
