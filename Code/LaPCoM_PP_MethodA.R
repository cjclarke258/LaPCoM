...
post_process_LaPCoM = function(output, # a list containing the output of the LaPCoM algorithm for each simulation
                           ...
                           cols = rep(khroma::color("light")(9), 4), seed = 123) {
  ...
  posterior_means_modes = list(alpha = rep(NA, length(output)),
                               ...
                               Kg_plus_hat = vector("list", length = length(output)),
                               pi = vector("list", length = length(output)),
                               w = vector("list", length = length(output)))
  ...
  for (sim in 1:length(output)) {
    ...
    posterior_means_modes$Kg_plus_hat[[sim]] = rep(NA, G_plus_hat)
    ...
    for (g in 1:G_plus_hat) {
      ...
      # 2. DETERMINE THE NUMBER OF NON-EMPTY COMPONENTS
      # -.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.
      Kg_plus_hat = params$Kg_plus_hat[[sim]][g] = posterior_means_modes$Kg_plus_hat[[sim]][g] = length(unique(posterior_means_modes$S[[sim]][[g]]))
      ...
      # -.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.
        ...
        if (Kg_plus_hat == 1) {
          ...
          for (t in 1:T_iters_node) {
            ...
            if (length(class_seqs_node[[t]]) == Kg_plus_hat) {
              
              if (all(sort(class_seqs_node[[t]]) == 1:Kg_plus_hat) == T) {
      ...
      if (Kg_plus_hat == 1) {
        ...
        for (t in 1:length(perm_indices_node)) {
          for (k in 1:Kg_plus_hat) {
            store_mus_node_level_g_relabelled[k, , t] = store_mus_node_level_g_active[[t]][which(class_seqs_keep_node[[t]] == k), ]
          } # end for k loop
        } # end t for loop
        chains$mu[[sim]][[g]] = store_mus_node_level_g_relabelled
        ...
        for (t in 1:length(perm_indices_node)) {
          for (k in 1:Kg_plus_hat) {
            store_Sigmas_node_level_g_relabelled[, , k, t] = store_Sigmas_node_level_g_active[[t]][, , which(class_seqs_keep_node[[t]] == k)]
          } # end for k loop
        } # end t for loop
        chains$Sigma[[sim]][[g]] = store_Sigmas_node_level_g_relabelled
        ...
        for (t in 1:length(perm_indices_node)) {
          for (k in 1:Kg_plus_hat) {
            store_pi_node_level_g_relabelled[t, k] = store_pi_node_level_g_active[[t]][which(class_seqs_keep_node[[t]] == k)]
          } # end for k loop
        } # end t for loop
    ...
    for (g in 1:posterior_means_modes$G_plus_hat[sim]) {
      ...
      if (is.null(posterior_means_modes$mu[[sim]][[g]])) {
        ...
        if (posterior_means_modes$Kg_plus_hat[[sim]][g] == 1) {
          ...
          empirical_means_g = matrix(NA, nrow = posterior_means_modes$Kg_plus_hat[[sim]][g], ncol = 2)
          closest_mean = rep(NA, posterior_means_modes$Kg_plus_hat[[sim]][g])
          
          for (k in 1:posterior_means_modes$Kg_plus_hat[[sim]][g]) {
            ...
            dists = rep(NA, posterior_means_modes$Kg_plus_hat[[sim]][g])
            
            for (kk in 1:posterior_means_modes$Kg_plus_hat[[sim]][g]) {
          ...
          posterior_means_modes_mu_reordered = matrix(NA, nrow = posterior_means_modes$Kg_plus_hat[[sim]][g], ncol = 2)
          posterior_means_modes_Sigmas_reordered = matrix(NA, nrow = posterior_means_modes$Kg_plus_hat[[sim]][g], ncol = 2)
          ...
          for (k in 1:posterior_means_modes$Kg_plus_hat[[sim]][g]) {
            ...
            if (all(is.na(posterior_means_modes_mu_reordered[k, ]))) {
              ...
              k_original = setdiff(1:posterior_means_modes$Kg_plus_hat[[sim]][g], closest_mean)
              
              mean_point = posterior_means_modes$mu[[sim]][[g]][k_original, ]
              covariance = matrix(c(posterior_means_modes$Sigma[[sim]][[g]][k_original, 1], 0, 0, posterior_means_modes$Sigma[[sim]][[g]][k_original, 2]), 2, 2, byrow = T)
              eigen_decomp = eigen(covariance)
              eigenvalues = eigen_decomp$values
              eigenvectors = eigen_decomp$vectors
              axis_length = sqrt(eigenvalues)
              ...
              points(x = posterior_means_modes$mu[[sim]][[g]][k_original, 1], y = posterior_means_modes$mu[[sim]][[g]][k_original, 2], 
                     pch = 15, col = "gray60", cex = 1.5) # draws WG
              
              points(x = posterior_means_modes$mu[[sim]][[g]][k_original, 1], y = posterior_means_modes$mu[[sim]][[g]][k_original, 2], 
                     pch = 7, col = "gray60", bg = "gray60", cex = 1.5)
              
              DescTools::DrawEllipse(x = posterior_means_modes$mu[[sim]][[g]][k_original, 1],
                                     y = posterior_means_modes$mu[[sim]][[g]][k_original, 2],
                                     radius.x = axis_length[1], radius.y = axis_length[2], border = "gray60", col = NA, lty = 2)
